...
docs["docs/ARCHITECTURE.md"] = r"""# 🛸 DroneNet-FPN-Attention: Architecture & Theoretical Foundations
...
1. **Extreme Small-Target Distribution ($< 32\text{px}$)**:
   - Analysis of the 2,400 raw frames (4,800 drone instances) reveals that **$95.42\%$ of all ground-truth bounding boxes are smaller than $32 \times 32$ pixels**, with **$51.52\%$ smaller than $16 \times 16$ pixels**.
...
3. **Severe Class Imbalance**:
   - Across a $640 \times 640$ spatial grid, the drone occupies less than $0.05\%$ of the total pixels, creating a $> 10,000 : 1$ ratio of background anchors to foreground targets.
...
| **Multi-Scale FPN** | ❌ (Single $\text{P}_3$ only) | ✅ ($\text{P}_2, \text{P}_3, \text{P}_4$ Top-Down) | ✅ High-Res FPN ($\text{P}_2, \text{P}_3, \text{P}_4$) |
...
| **Anchor Scales** | 3 anchors ($\text{P}_3$) | 9 anchors (3 per scale) | 9 calibrated multi-scale anchors |
...
Unlike standard FPNs that operate on strides $\{8, 16, 32\}$ ($\text{P}_3, \text{P}_4, \text{P}_5$), our **High-Resolution FPN** shifts receptive attention to strides $\{4, 8, 16\}$ ($\text{P}_2, \text{P}_3, \text{P}_4$):

- **$\text{P}_2$ (Stride 4, Resolution $160 \times 160$)**: Dedicated to tiny drones ($4\text{px} - 24\text{px}$). Preserves microscopic spatial features such as rotor tips and landing skids.
- **$\text{P}_3$ (Stride 8, Resolution $80 \times 80$)**: Dedicated to medium-scale drones ($24\text{px} - 64\text{px}$). Balances semantic context and localization precision.
- **$\text{P}_4$ (Stride 16, Resolution $40 \times 40$)**: Dedicated to larger, close-range UAVs ($> 64\text{px}$). Captures structural fuselage context.
...
$$\mathbf{z}_c^h(h) = \frac{1}{W} \sum_{i=0}^{W-1} x_c(h, i), \quad \mathbf{z}_c^w(w) = \frac{1}{H} \sum_{j=0}^{H-1} x_c(j, w) \tag{1}$$
...
3. Two independent $1 \times 1$ convolutions and sigmoid ($\sigma$) activations yield coordinate attention weights:
   $$\mathbf{g}^h = \sigma\left(\text{Conv}_h(\mathbf{f}^h)\right), \quad \mathbf{g}^w = \sigma\left(\text{Conv}_w(\mathbf{f}^w)\right) \tag{2}$$
4. The output feature map is reweighted along both orthogonal directions:
   $$\mathbf{y}_c(i, j) = x_c(i, j) \times \mathbf{g}_c^h(i) \times \mathbf{g}_c^w(j) \tag{3}$$
...
$$\mathcal{L}_{\text{total}} = \lambda_{\text{obj}} \mathcal{L}_{\text{obj}} + \lambda_{\text{box}} \mathcal{L}_{\text{box}} + \lambda_{\text{cls}} \mathcal{L}_{\text{cls}} \tag{4}$$
...
$$\mathcal{L}_{\text{obj}} = -\alpha_t (1 - p_t)^\gamma \log(p_t) \tag{5}$$
...
$$\mathcal{L}_{\text{box}} = 1 - \text{IoU} + \frac{\rho^2(\mathbf{b}, \mathbf{b}^{\text{gt}})}{c^2} + \alpha_{\text{ciou}} v \tag{6}$$

where:

$$v = \frac{4}{\pi^2}\left(\arctan\frac{w^{\text{gt}}}{h^{\text{gt}}} - \arctan\frac{w}{h}\right)^2, \quad \alpha_{\text{ciou}} = \frac{v}{(1 - \text{IoU}) + v} \tag{7}$$

### 4.3 Label-Smoothed Classification Loss ($\mathcal{L}_{\text{cls}}$)

$$\mathcal{L}_{\text{cls}} = -\sum_{k=1}^K \left[ y_k^{\text{ls}} \log(\hat{c}_k) + (1 - y_k^{\text{ls}}) \log(1 - \hat{c}_k) \right] \tag{8}$$
...
$$\eta_t = \eta_{\text{min}} + \frac{1}{2}(\eta_0 - \eta_{\text{min}})\left(1 + \cos\left(\frac{t}{T_{\text{max}}}\pi\right)\right) \tag{9}$$
"""

docs["docs/KAGGLE_DUAL_GPU_GUIDE.md"] = r"""# ☁️ Kaggle Dual-GPU Execution Guide
...
docs["docs/GETTING_STARTED.md"] = r"""# 🚀 Getting Started Guide
...
docs["docs/BENCHMARKS_AND_EVALUATION.md"] = r"""# 📊 Benchmarks, Ablation Studies & Evaluation
...
docs["README.md"] = r"""# 🛸 DroneNet-FPN-Attention: A Lightweight Multi-Scale Receptive-Field Attention Network for Scratch UAV Detection Under Adverse Atmospheric Conditions
