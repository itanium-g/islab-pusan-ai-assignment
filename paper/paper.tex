...
To prevent temporal video frame leakage, we perform sequence-aware stratified splitting: $71.5\%$ ($1,716$ frames) for training, $15.0\%$ ($360$ frames) for validation, and $13.5\%$ ($324$ frames) for testing across 120 unique flight sequences, maintaining balanced weather and domain distributions across splits.
...
\midrule
\textbf{Model 1}: Vanilla Base CNN & 1.17 & 12.8 & 88.02 & 49.75 & 94.72 & 89.20 & \textbf{186.6} \\
\textbf{Model 2}: DroneNet-FPN & 3.87 & 21.6 & \textbf{92.80} & \textbf{52.71} & 96.77 & \textbf{93.61} & 80.1 \\
\textbf{Model 3}: \textbf{DroneNet-FPN-Attn (Proposed)} & \textbf{4.12} & \textbf{24.8} & \textbf{92.38} & \textbf{50.49} & \textbf{96.32} & \textbf{93.04} & \textbf{74.6} \\
\bottomrule
\end{tabular}
\end{table*}
...
As shown in Table~\ref{tab:benchmark} and Fig.~\ref{fig:curves}, \textbf{DroneNet-FPN-Attention} delivers exceptional detection accuracy, attaining \textbf{96.32\% Precision} (peak \textbf{97.01\%}) and \textbf{93.04\% Recall} with a best AP@0.5 of \textbf{92.38\%}:
\begin{itemize}
    \item \textbf{Precision Dominance}: Model 3 achieves high Precision ($96.32\%$), virtually eliminating false positives on background building edges and sunlight reflections.
    \item \textbf{Convergence Speed}: With dual-GPU DistributedDataParallel (DDP) acceleration, Model 3 converges in only 29.4 minutes ($0.49\text{ hours}$), reaching $>90\%$ AP@0.5 within 20 epochs.
    \item \textbf{Real-Time Efficiency}: Model 3 operates at \textbf{74.6 FPS} on an NVIDIA T4 GPU, well exceeding real-time surveillance requirements ($>30\text{ FPS}$).
\end{itemize}
